...
contactsync.factory('contactSync', function ($http) {
    ...
    returnval.create = function (data, callback) {
        ...
        returnval.query("INSERT OR REPLACE INTO `contacts` (`id`, `name`,`email`,`personalMobile`) VALUES (null,'" + data.name + "','" + data.email + "','" + data.contact + "')", function (result, len, id) {
            id = id.insertId;
            var d = new Date();
            var n = d.getTime();

            returnval.query("INSERT OR REPLACE INTO `userslog` (`id`,`timestamp`,`type`,`user`,`table`) VALUES (null,'" + n + "','" + 1 + "','" + user + "','" + id + "')", null);
            //           console.log(id);
            if (callback) {
                callback();
            }
